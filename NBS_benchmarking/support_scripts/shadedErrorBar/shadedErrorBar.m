...
% obtained from https://github.com/raacampbell/shadedErrorBar and lightly modified by sNeuroble for network-based inference benchmarking
...
% 'noLine' - [true  by default] if true, the main line is cleared
%            (added by SMN 030320)
...
params.addParameter('noLine', false, @(x) islogical(x) || x==0 || x==1); % SMN
...
H = makePlot(x,y,errBar,lineProps,noLine,transparent,patchSaturation);
...
function H = makePlot(x,y,errBar,lineProps,noLine,transparent,patchSaturation) % SMN
    ...
    % Remove mainline if specified - SMN
    if noLine
        H.mainLine.Color(4)=0;
    end
