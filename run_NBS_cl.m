...
% Note: if interested in cluster stats, compare nbs.NBS.cluster_stats to nbs.STATS.edge_groups.unique to reorder
if strcmp(cluster_stat_type,'Constrained')
    
    % Visualize significant networks
    if exist('nbs','var')
        ...
        % assuming data is in original order so reorder
        draw_atlas_boundaries(sig_edge_results,'reorderimg',1);
    end
